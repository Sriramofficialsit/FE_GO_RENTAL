...
const CarFormWithPreview = () => {
  ...
  const handleFileChange = (e, fileType) => {
    ...
    if (file) {
      if (fileType === "image") {
      ...
      } else if (fileType === "insurance" || fileType === "rc_book") {
        if (file.type !== "application/pdf") {
          setError("Please upload a PDF file for insurance or RC book.");
          return;
        }
        if (file.size > 5 * 1024 * 1024) {
        ...
        if (fileType === "insurance") setInsuranceFile(file);
        else setRcBookFile(file);
      }
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      setError(
        "All fields are required, including image, insurance PDF, RC book PDF, and permitted city."
      );
      return;
    }
    ...
    data.append("car_model", formData.car_model.trim());
    ...
    data.append("rc_book", rcBookFile);

    try {
      setIsSubmitting(true);
      const response = await axios.post(
        "https://be-go-rental-hbsq.onrender.com/api/approvals/requests-insert",
        data,
        {
          headers: {
      ...
      if (response.data.success) {
        setSuccess("Car request submitted successfully! Awaiting approval.");
        setTimeout(() => navigate("/vehicles"), 2000);
      } else {
        setError(
          response.data.message || "Failed to submit request: Unknown error"
        );
      }
    } catch (error) {
      console.error("Error submitting car request:", {
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div>
                  ...
                  <input
                    ...
                    readOnly
                    className="mt-1 w-full border rounded-lg px-4 py-2 text-gray-500 bg-gray-100 cursor-not-allowed"
                    placeholder="e.g., ravikumar@example.com"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700">
                    Car Image (PNG, JPG, JPEG, max 5MB)
                  </label>
                  <input
                    type="file"
                ...
                <div>
                  ...
                  <input
                    type="file"
                    accept="application/pdf"
                    onChange={(e) => handleFileChange(e, "rc_book")}
                    className="mt-1 w-full border rounded-lg px-4 py-2 text-gray-700 bg-white file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100"
                  />
                </div>
              ...
              >
                ...
                <div className="flex items-center justify-center p-4 bg-gray-50">
                  <motion.img
                    ...
                    alt={formData.car_model || "Car preview"}
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-3 px-4 py-3 text-gray-600 text-sm">
                  {[
                    ...
                    <motion.div
                      ...
                      custom={index}
                    >
                      <img src={info.icon} alt={info.alt} className="w-5 h-5" />
