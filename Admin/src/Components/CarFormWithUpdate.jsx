...
const CarFormWithUpdate = () => {
  ...
  const getCurrentDateTime = () => {
    const now = new Date("2025-06-16T23:06:00+05:30"); // Current date and time: June 16, 2025, 11:06 PM IST
      ...
      try {
        ...
        if (response.data.success && response.data.car) {
          ...
          setFormData({
            ...
            from: car.from ? new Date(car.from).toISOString().slice(0, 16) : "",
            to: car.to ? new Date(car.to).toISOString().slice(0, 16) : "",
  ...
  const handleSubmit = async (e) => {
    ...
    // Fetch original car data for comparison
    let originalCarData = {};
    try {
      const response = await axios.get(
        `https://be-go-rental-hbsq.onrender.com/dashboard/cars/${carId}`,
      ...
      if (response.data.success && response.data.car) {
        originalCarData = response.data.car;
      }
    } catch (err) {
      console.error("Error fetching original car data:", err);
    }

    const data = new FormData();

    // Only add fields that have changed or are non-empty
    const fields = [
      { key: "name", value: formData.name },
      { key: "priceperday", value: formData.priceperday },
      { key: "ac", value: formData.ac },
      { key: "passengers", value: formData.passengers },
      { key: "transmission", value: formData.transmission },
      { key: "seats", value: formData.seats },
      { key: "doors", value: formData.doors },
      { key: "modelYear", value: formData.modelYear },
      { key: "ratings", value: formData.ratings },
      { key: "reviews", value: formData.reviews },
      { key: "fuelType", value: formData.fuelType },
      { key: "carNumber", value: formData.carNumber },
      { key: "from", value: formData.from },
      { key: "to", value: formData.to },
      { key: "permited_city", value: formData.permited_city },
    ];
    ...
    for (const { key, value } of fields) {
      ...
      if (
        ...
        value === undefined ||
        (originalCarData[key] &&
          value.toString() === originalCarData[key].toString())
      ) {
        continue;
      }

      // Validate non-empty fields
      if (key === "priceperday" && (isNaN(value) || value <= 0)) {
        setError("Price per day must be a positive number.");
        return;
      }
      if (key === "passengers" && (isNaN(value) || value < 1 || value > 8)) {
        setError("Passengers must be a number between 1 and 8.");
        return;
      }
      if (key === "seats" && (isNaN(value) || value < 1 || value > 8)) {
        setError("Seats must be a number between 1 and 8.");
        return;
      }
      if (key === "doors" && (isNaN(value) || value < 1 || value > 6)) {
        setError("Doors must be a number between 1 and 6.");
        return;
      }
      if (
        key === "modelYear" &&
        (isNaN(value) || value < 1900 || value > new Date().getFullYear() + 1)
      ) {
        setError(
      ...
      if (key === "from" || key === "to") {
        ...
        if (isNaN(date.getTime())) {
          setError(`Invalid ${key} date.`);
          return;
        }
      }
      if (key === "permited_city") {
        const validCities = [
          "chennai",
          "coimbatore",
          "madurai",
    ...
    console.log("Sending FormData for car-update:");
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setError(
          "Failed to update car: " + (response.data.message || "Unknown error")
        );
      }
    } catch (error) {
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="mt-1 w-full border rounded-lg px-4 py-2 text-gray-700 bg-white file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 transition"
                  />
                </div>
              ...
              >
                ...
                <div className="flex items-center justify-center p-4 bg-gray-50">
                  {imageLoading ? (
                    ...
                    <motion.img
                      src={
                        imagePreview || "https://via.placeholder.com/200x150"
                      }
