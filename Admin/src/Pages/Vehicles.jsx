...
const Vehicles = () => {
    ...
    const fetchVehicles = async () => {
      try {
        const response = await axios.get(
          "https://be-go-rental-hbsq.onrender.com/dashboard/cars",
          {
            headers: {
  ...
  return (
    <>
      <SideBar />
      <div className="pt-[13vh] min-h-screen bg-gray-100 flex flex-col">
        <div className="flex flex-col w-full">
          {/* Desktop Sidebar */}
          <div className="hidden lg:flex lg:flex-col lg:w-64 lg:min-h-screen lg:fixed lg:border-r lg:pr-4 lg:gap-4 lg:pt-8 lg:pl-4 lg:bg-gray-200 lg:shadow-md">
            <SelectFilter
              name="seats"
              value={filters.seats}
            ...
            <div className="flex flex-col gap-2">
              <button
                className="bg-black rounded px-4 py-2 text-white hover:bg-gray-800 transition-colors duration-200"
                onClick={handleclick}
              >
                Add New
              </button>
          ...
          <div
            className={`lg:hidden w-full bg-gray-200 border-b rounded shadow-md transition-all duration-300 ease-in-out ${
              isOpen ? "min-h-[50vh]" : "h-14"
            }`}
          >
            <div
              className="flex items-center justify-between px-4 py-3 cursor-pointer hover:bg-gray-300 transition-colors duration-200"
              onClick={() => setIsOpen(!isOpen)}
            >
              <div className="font-semibold text-gray-800 text-lg">FILTERS</div>
              <div>
                {isOpen ? (
            ...
            {isOpen && (
              <div className="flex flex-col px-4 py-4 gap-3 bg-gray-200">
                <SelectFilter
                  name="seats"
                  value={filters.seats}
                  onChange={handleFilterChange}
          ...
          {/* Main Content */}
          <div className="lg:ml-64 p-4 sm:p-6 lg:p-6 w-full lg:w-[calc(100%-16rem)] max-w-[1200px] mx-auto">
            {error ? (
              <p className="text-red-600 text-center text-sm sm:text-base mb-4">
                {error}
              </p>
            ) : filteredVehicles.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-4 sm:gap-6 lg:gap-6">
...
const SelectFilter = ({ name, value, onChange, options }) => (
  <select
    ...
    value={value}
    onChange={onChange}
    className="border rounded px-3 py-2 sm:px-4 sm:py-2 text-gray-700 bg-white hover:outline hover:outline-indigo-500 focus:outline-indigo-500 transition-all w-full text-sm sm:text-base"
  >
